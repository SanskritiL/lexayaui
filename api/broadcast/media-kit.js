    ...
    switch (req.method) {
        ...
        case 'POST':
            ...
            if (req.query.action === 'insights') {
                return handleInstagramInsights(req, res, supabase, user);
            }
...
// Instagram Insights
async function handleInstagramInsights(req, res, supabase, user) {
    console.log('[MediaKit] Fetching Instagram insights for user:', user.id);
    ...
    try {
        ...
        console.error('[MediaKit] Insights error:', error);
...
async function fetchAudienceInsights(igUserId, accessToken) {
    try {
        const metrics = ['audience_city', 'audience_country', 'audience_gender_age', 'audience_locale'].join(',');
        ...
        for (const item of (data.data || [])) {
            ...
            if (name === 'audience_city') {
                insights.top_cities = Object.entries(values).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([city, count]) => ({ city, count }));
            } else if (name === 'audience_country') {
                insights.top_countries = Object.entries(values).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([country, count]) => ({ country, count }));
...
async function fetchAccountMetrics(igUserId, accessToken) {
    try {
        const metrics = ['impressions', 'reach', 'profile_views', 'website_clicks'].join(',');
        const since = Math.floor((Date.now() - 30 * 24 * 60 * 60 * 1000) / 1000);
        const until = Math.floor(Date.now() / 1000);
        const url = `https://graph.facebook.com/v18.0/${igUserId}/insights?metric=${metrics}&period=day&since=${since}&until=${until}&access_token=${accessToken}`;
        const response = await fetch(url);
        const data = await response.json();
