...
window.CONFIG = {
    ...
    // Left undefined unless a deployment overrides it, so js/api-config.js can
    // fall back to http://localhost:8080 when the site is served from the local
    // static preview. In production, Firebase Hosting rewrites /api to Cloud
    // Run, so same-origin ('') is what api-config.js resolves to anyway.
    ...
    // Display copy only. Stripe price IDs live in api/_plans.js so the browser
    // cannot choose which price it pays; checkout is called with a plan key.
    PLANS: {
        dm: { name: 'Get Lexaya', price: '$7.99', period: 'per month' },
        pro: { name: 'Lexaya Pro', price: '$25', period: 'per month' },
    },
};
